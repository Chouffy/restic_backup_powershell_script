...
Write-Output "Send start signal to HealthChecks ..."; Invoke-RestMethod ($HealthChecksForgetLink+"/start")
...
if(-not $?) { Write-Output "`nSend fail signal to HealthChecks ..."; Invoke-RestMethod ($HealthChecksForgetLink+"/fail")} else {Write-Output "`nSend success signal to HealthChecks ..."; Invoke-RestMethod $HealthChecksForgetLink}
